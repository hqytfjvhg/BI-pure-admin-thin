    ...
    >
      <el-card class="bar-card" shadow="never">
        <div class="flex">
          ...
          <span style="color: #62c23a; margin-left: 10px" v-if="barDate != ''"
            >{{ t("salesBoard.common.dateStr")
            }}{{ locale === "zh" ? barDate[0] : transformDate(barDate[0]) }}~{{
              locale === "zh" ? barDate[1] : transformDate(barDate[1])
            }}</span
          >
        </div>
...
const receivableBar = arr => {
  ...
  arr.forEach(op => {
    if (salesBarOne.value == "US$m") {
      ...
      yData.push(op.totalData);
    } else if (salesBarOne.value == "CNY") {
      yData.push((op.totalData * cnyRate).toFixed(2));
    }
...
const getData = () => {
  getSalesData().then(res => {
    barDate.value = [res.data.startDate, res.data.endDate];
    twentyArr.value = res.data.countrySaleDataList;
