...
import { routerArrays } from "@/utils/responsive";
...
/** 从localStorage里取出当前登录用户的角色roleName，过滤无权限的菜单 */
function filterNoPermissionTree(data) {
  const currentRoles = storageLocal().getItem(userKey)?.roleName ?? "";
...
/** 初始化路由（`new Promise` 写法防止在异步请求中造成无限循环），获取前后端路由,暂时未用到*/
function initRouter() {
...
function getTopMenu(tag = false) {
  const topMenu = handleTopMenu(
    usePermissionStoreHook().constantMenus[0]?.children[0]
  );
