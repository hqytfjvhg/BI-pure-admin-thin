...
const menus = routes => {
  return routes.filter(route => {
    ...
    //判断一级权限
    if (
      route?.children?.every(item => item.children?.length == 0) &&
      route.path !== "/"
    ) {
      hasAccess = false;
    }
...
const menuData = computed(() => {
  ...
  // return usePermissionStoreHook().constantMenus
  let arr =
    pureApp.layout === "mix"
      ? getSubMenuData()
      : menus(filterTree(ascending(usePermissionStoreHook().constantMenus)));
  arr.forEach(op => {
    if (op.path == "/") {
      op.path = "/overview";
    }
