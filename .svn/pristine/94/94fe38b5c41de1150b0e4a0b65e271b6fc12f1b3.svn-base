...
import { getMonthEnd, transformDate } from "@/utils/transformDate";
...
//日期
const dateOneTitle = ref("");
const handleChangeOneDate = val => {
  dateOneTitle.value = getMonthEnd(val);
};
const dateTwoTitle = ref("");
const handleChangeTwoDate = val => {
  dateTwoTitle.value = getMonthEnd(val);
};
...
onMounted(() => {
  ...
  if (regionList.value.length === 0 && fourAreaList.value.length === 0) {
    searchOneForm.distributorIdList = fiveAreaList.value;
  ...
  if (regionList.value.length === 0 && fourAreaList.value.length > 0) {
    ...
    searchOneForm.regionSelect = "国家";
    searchTwoForm.regionSelect = "国家";
    searchOneForm.countryList.push(fourAreaList.value[0]?.resourceId);
  ...
  handleChangeOneDate(new Date());
  handleChangeTwoDate(new Date());
