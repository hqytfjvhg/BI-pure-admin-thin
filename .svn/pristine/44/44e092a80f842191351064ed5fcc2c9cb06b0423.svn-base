...
const paintPieTwo = arr => {
  ...
  myPieTwo.setOption({
    title: {
      text: t("home.lastMonth") + pieTwoDate.value,
      textStyle: {
        fontSize: 16,
        fontWeight: "normal"
      },
...
const paintBarOne = arr => {
  ...
  myBarOne.setOption({
    title: {
      text:
        t("home.topDistributor") +
        ("(" + barOneData.value.start + "~" + barOneData.value.end + ")"),
      left: "center",
      top: "bottom",
      textStyle: {
        fontSize: 16,
      ...
      feature: {
        myTool1: {
          ...
          onclick: e => {
            yNameOne.value = "US$m";
            // getBarOne();
            paintBarOne(barOneData.value.data);
          }
        },
        myTool2: {
          show: true, //是否显示
...
const paintBarTwo = arr => {
  ...
  myBarTwo.setOption({
    title: {
      text: t("home.topProduct") + barTwoDate.value,
      left: "center",
      top: "bottom",
      textStyle: {
        fontSize: 16,
    ...
    yAxis: {
      type: "value",
      name: viewShow ? yNameTwo.value : t("status.salesQuantity"),
      axisLabel: {
        fontSize: 14
      }
    },
...
const pieTwoDate = ref("");
const getPieTwo = () => {
  getLast12MonthTotalSaleMoneyTop10Product().then(res => {
    pieTwoDate.value = "(" + res.data.start + "~" + res.data.end + ")";
...
const barTwoDate = ref("");
const getBarTwo = () => {
  getTotalSaleMoneyTop10Product().then(res => {
    barTwoDate.value = "(" + res.data.start + "~" + res.data.end + ")";
                    ...
                    >
                      ...
                      <div>
                        {{
                          (
                            ((
                              threeValue.currentYearData?.data / 1000000
                            ).toFixed(2) -
                              (threeValue.lastYearData?.data / 1000000).toFixed(
                                2
                              )) /
                            (threeValue.lastYearData?.data / 1000000).toFixed(2)
                          ).toFixed(4) * 100
