  ...
  <div>
    <el-form :inline="true">
      <el-form-item>
        ...
        <el-button type="success" @click="handleAddFunctionally">{{
          t("permission.product.addFunctionally")
        }}</el-button> -->
        <el-button
          type="success"
          @click="handleAdd(t('permission.product.addSeries'))"
          >{{ t("permission.product.addSeries") }}</el-button
        >
        <el-button
          type="success"
          @click="handleAdd(t('permission.product.addFunction'))"
          >{{ t("permission.product.addFunction") }}</el-button
        >
        <el-button
          type="success"
          @click="handleAdd(t('permission.product.addFunctionally'))"
          >{{ t("permission.product.addFunctionally") }}</el-button
        >
      </el-form-item>
...
const attribute = reactive([
  ...
  { title: "permission.product.functionally", name: "functionally" }
]);

//获取系列和功能（产品类型）的表格数据，还有功能类型
...
const getAllAttributes = async () => {
  ...
  await getAllFunctionally().then(res => {
    if (res.success) {
      allAttribute.value.push(res.data);
    }
  });
...
// //新增功能类型
// const handleAddFunctionally = () => {
//   if (ruleForm.value) {
//     ruleForm.value.resetFields();
//   }
//   dialog.show = true;
//   dialog.title = t("permission.product.addFunctionally");
//   editFormData.value = {};
//   type.value = null;
// }
      ...
      ruleForm.value.validate(valid => {
        if (valid) {
          ...
          if (dialog.title == t("permission.product.addSeries")) {
            ...
            handleAxios("/productClassification/addSeries", editFormData.value);
          } else if (dialog.title == t("permission.product.addFunction")) {
            //产品类型
            activeName.value = "function";
            ...
            handleAxios(
              "/productClassification/addFunction",
              editFormData.value
            );
          } else if (dialog.title == t("permission.product.addFunctionally")) {
            //功能类型
            activeName.value = "functionally";
            handleAxios(
              "/productClassification/addFunctionally",
              editFormData.value
            );
          } else {
            //修改
            if (type.value == "series") {
              ...
              // .finally(resolve);
              handleAxios(
                "/productClassification/updateSeries",
                editFormData.value
              );
              ...
              // .finally(resolve);
              handleAxios(
                "/productClassification/updateFunction",
                editFormData.value
              );
            } else {
              //功能类型
              handleAxios(
                "/productClassification/updateFunctionally",
                editFormData.value
              );
            }
          }
...
//请求传地址，新增和修改
const handleAxios = (url, data) => {
  addOrUpdate(url, data).then(res => {
    if (res.success) {
      ElMessage.success(t("status.pureSuccess"));
      dialog.show = false;
      getAllAttributes();
...
const handleDel = (row, attributeType) => {
  ...
  ElMessageBox.confirm(t("status.delReg"), t("status.warn"), {
    ...
    if (attributeType === "series") {
      ...
      delFunctionally([row.id]).then(res => {
        if (res.success && res.data) {
          ElMessage.success(t("status.pureSuccess"));
          getAllAttributes();
        }
