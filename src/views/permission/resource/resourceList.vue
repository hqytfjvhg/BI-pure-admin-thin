    ...
    <el-tabs v-model="activeName" class="demo-tabs">
      ...
      >
        ...
        >
          ...
          <el-table-column
            v-if="index !== 3"
            :label="t('permission.resource.enName')"
            prop="enName"
          />
          <el-table-column
            :label="t('permission.resource.lineLevel')"
            :prop="locale === 'zh' ? 'parentName' : 'enParentName'"
          />
          <el-table-column
            v-if="index === 3"
            :label="t('permission.resource.people')"
            prop="population"
          />
          <el-table-column
            v-if="index === 3"
            :label="t('permission.resource.perCapita')"
    ...
    <Dialog :dialog="dialog" @confirm="submitResource">
      ...
      >
        ...
        <el-form-item
          ...
          v-if="addFormData.level === 4 && type == 'edit'"
        >
          <el-input v-model="addFormData.population" />
        </el-form-item>
        <el-form-item
          :label="t('permission.resource.perCapita')"
          prop="perCapitaIncome"
          v-if="addFormData.level === 4 && type == 'edit'"
        >
          <el-input v-model="addFormData.perCapitaIncome" />
        </el-form-item>
...
const rules = computed(() => {
  return {
    ...
    enName: [
      {
        required: true,
        message: t("permission.resource.enNameReg"),
        trigger: "blur"
      }
    ]
    // remark: [
...
const transData = data => {
  return data.map(item => {
    const transformedItem = {
      ...
      label: locale.value === "zh" ? item.name : item.enName,
...
const handleDel = (row, str) => {
  ...
  ElMessageBox.confirm(t("status.delReg"), t("status.warn"), {
    ...
    .then(() => {
      deleteResourceByResourceId(getId([row])).then(res => {
