...
export const useMultiTagsStore = defineStore({
  id: "pure-multiTags",
  state: () => ({
    // 存储标签页信息（路由信息）,formatFlatteningRoutes是转换为一维数组，usePermissionStoreHook().constantMenus获取静态嵌套路由
    // multiTagsCache是指标签持久化，若持久刷新页面就会从storageLocal里面，若选择否就只有首页会显示

    multiTags: storageLocal().getItem("tagMenus") || [],
    ...
    //   `${responsiveStorageNameSpace()}configure`
    // )?.multiTagsCache
    //   ? storageLocal().getItem("tagMenus")
    //   : [
    //       ...routerArrays,
    //       ...formatFlatteningRoutes(
    //         usePermissionStoreHook().constantMenus
